...
var obj={value:1};
function a(obj1){
  /*命名参数obj1是局部变量,当我传递进去一个引用类型的对象后
  *其实局部变量obj1={value:1},然后局部变量将obj1.value=1赋值成obj1.value=2
  *因为全局obj与obj1这个局部变量引用的地址是同一个
  *所以全局变量的值被改变了；
  */
  obj1.value=2;
  /*因为obj1是局部变量，又被赋值成基础类型的值,所以在这输出时，值就变为2
  * 如果这边局部变量obj1={},等于内存中又增加了一个新的空间
  * 与全局变量obj并不是同一个引用地址
  * 所以并不会改变全局obj的引用的值
  */
  // obj1={};
  //当局部变量obj1，在函数a()执行完成后，就会被立马销毁
  console.log(obj1);
  console.log(obj);
  /*局部变量obj1在函数执行完成后会立即销毁
  *如果需要使用局部变量
